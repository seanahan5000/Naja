...
%ASMB% NAJA\IO		MBOOT.S			-list %ROOT%\MBOOT.LST
%ASMB% NAJA\IO		FAKE.GROUP.S
%ASMB% NAJA\IO		ASM.NCOMMONS.S	-list %ROOT%\NCOMMONS.LST

%ASM% NAJA\IO				ASM.CHAR.DATA.S
%ASM% NAJA\BOOT				PR.INFO.S
%ASM% TRACC					ASM.CAMP.S

%ASM% MOTHER\HALL			ASM.HALL.S		-list %ROOT%\MCONTROL.LST
...
%ASMB% NAJA\IO		NDOS.525.S		-list %ROOT%\NDOS.525.LST
